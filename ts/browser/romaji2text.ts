// romaji2text.ts — ストリーミング型 ローマ字→テキスト変換エンジン (TypeScript)
...
//
// Usage (Node):
//   const { Romaji2Text } = require("./romaji2text");

export type TextEventType = "input" | "converted" | "confirmed" | "correction";
...
type EventCallback = (ev: TextEvent) => void;
...
// ── Builtin Dictionary ──────────────────────────────────────────────────────
const BUILTIN_DICT = new Set<string>([
...
export class Romaji2Text {
  ...
  private committed = "";
  private dict = new Set<string>(BUILTIN_DICT);
  ...
  // ── イベントリスナ ──────────────────────────────────────────────────────
  on(type: TextEventType, cb: EventCallback): void {
    if (!this.listeners.has(type)) this.listeners.set(type, []);
    this.listeners.get(type)!.push(cb);
  }

  off(type: TextEventType, cb: EventCallback): void {
    const arr = this.listeners.get(type);
    if (arr) this.listeners.set(type, arr.filter(c => c !== cb));
  }

  private emit(ev: TextEvent): void {
    const cbs = this.listeners.get(ev.type) ?? [];
    cbs.forEach(cb => cb(ev));
    // Also emit on "*" for catch-all
    const all = this.listeners.get("*") ?? [];
    all.forEach(cb => cb(ev));
  }

  // ── キーストローク受信 ──────────────────────────────────────────────────
  onKey(ch: string | KeyboardEvent): TextEvent {
    let c: string;
    if (typeof ch === "string") {
      c = ch;
    } else {
  ...
  getCommitted(): string { return this.committed; }
  ...
  // ── カスタム辞書 ────────────────────────────────────────────────────────
  addDictEntry(yomi: string): void { this.dict.add(yomi); }
  ...
  // ── プライベート ────────────────────────────────────────────────────────
  private makeInputEvent(): TextEvent {
    return {
      type: "input",
  ...
  // ── Romaji → Hiragana ───────────────────────────────────────────────────
  private convertToken(src: string): string {
    src = src.toLowerCase();
    let out = "";
    let i = 0;
